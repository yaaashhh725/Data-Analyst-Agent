...
def debug_code(task: dict,last_task_output:str, failed_code: str, error_message: str) -> str:
    client = genai.Client(
...
2. **Previous task output**: The output of previous task for better context at this step.
3.  **FAILED PYTHON SCRIPT:** The complete source code that resulted in an error.
4.  **EXECUTION ERROR TRACEBACK:** The full standard error output (`stderr`) from the failed execution, which includes the error type and traceback.
